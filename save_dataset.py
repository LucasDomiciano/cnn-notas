...
save_image = False
initial_count = 1  # Contador inicial
final_count = 30   # Contador final
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Salvar a imagem da mão quando a tecla 's' for pressionada e o contador estiver no intervalo desejado
            if save_image:
                initial_count = image_count
                final_count = initial_count + 29

                while initial_count <= final_count:
                    cv2.imwrite("./dataset/D/" + str(initial_count) + ".jpg", cv2.cvtColor(hand_image, cv2.COLOR_RGB2BGR))
                    initial_count += 1
                print("Imagem da mão salva!")

                image_count += initial_count + 1
    ...
    # Sair do loop quando o contador atingir o valor final
    #if image_count > final_count:
        #break
