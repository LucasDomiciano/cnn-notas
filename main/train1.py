...
for class_name in classes:
    ...
    data.extend([(os.path.join(class_path, img), class_name) for img in images])

# Separar dados em conjuntos de treinamento e teste
train_data, test_data = train_test_split(data, test_size=0.25, random_state=42)

# Converter listas de tuplas para listas separadas de imagens e classes
train_images, train_labels = zip(*train_data)
test_images, test_labels = zip(*test_data)


# Configurar geradores de dados usando ImageDataGenerator
...
# Gerar conjuntos de treinamento e teste usando flow_from_dataframe
train_set = train_datagen.flow_from_dataframe(
    dataframe=pd.DataFrame({'filename': train_images, 'class': train_labels}),
    directory=None,  # Neste caso, o caminho já está incluído nos caminhos das imagens
    target_size=(150, 150),
    color_mode='rgb',
    batch_size=32,
...
model.compile(optimizer=SGD(0.001), loss="categorical_crossentropy",
              metrics=["accuracy"])
